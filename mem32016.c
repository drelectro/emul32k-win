...
uint8_t ns32016mem[SYSTEM_SIZE];

void init_ram(void)
{
   memset(ns32016mem, 0, sizeof(ns32016mem));
...
uint16_t read_x16(uint32_t addr)
{
   ...
   if (addr < IO_BASE)
   {
      return *((uint16_t*) (ns32016mem + addr));
   }
#endif
...
uint32_t read_x32(uint32_t addr)
{
   ...
   if (addr < IO_BASE)
   {
      return *((uint32_t*) (ns32016mem + addr));
   }
#endif
...
uint32_t read_n(uint32_t addr, uint32_t Size)
{
   if (Size <= sizeof(uint64_t))
   {
      ...
      {
         uint32_t Result = 0;
         memcpy(&Result, ns32016mem + addr, Size);
         return Result;
      }
   }
...
//	n32016_ShowRegs(0xFF);
   ...
   if (addr <= (SYSTEM_SIZE - sizeof(uint8_t)))
   {
       ns32016mem[addr] = val;
      return;
   }
...
#ifdef NS_FAST_RAM
   if (addr <= (SYSTEM_SIZE - sizeof(uint16_t)))
   {
      *((uint16_t*) (ns32016mem + addr)) = val;
      return;
   }
#endif
...
#ifdef NS_FAST_RAM
   if (addr <= (SYSTEM_SIZE - sizeof(uint32_t)))
   {
      *((uint32_t*) (ns32016mem + addr)) = val;
      return;
   }
#endif
...
#ifdef NS_FAST_RAM
   if (addr <= (SYSTEM_SIZE - sizeof(uint64_t)))
   {
      *((uint64_t*) (ns32016mem + addr)) = val;
      return;
   }
#endif
...
#ifdef NS_FAST_RAM
   if ((addr + Size) <= SYSTEM_SIZE)
   {
      memcpy(ns32016mem + addr, pData, Size);
      return;
   }
#endif
...
uint32_t LoadBinary(const char *pFileName, uint32_t Location)
{
   ...
   if (pFile)
   {
      ...
      { 
         End = fread(ns32016mem + Location, sizeof(uint8_t), FileSize, pFile) + Location;
      }
